...
Expr ConcatenateQnnCanonicalize(const Attrs& attrs, const Array<Expr>& new_args,
  ...
  if (input_scales->IsInstance<CallNode>()) // for example, input_scales is CallNode(Op(device_copy)...
  {
    ...
    for (auto quantized_expr : tuple_exprs) {
      ...
      std::cout << "input_scale in CallNode: " << input_scale << std::endl;

      // Get the zero point for the idx quantized input tensor.
      auto input_zero_point = tuple_input_zero_points->fields[idx];
      std::cout << "input_zero_point in CallNode: " << input_scale << std::endl;
      ...
      if (!IsEqualScalar(input_scale, output_scale) ||
        ...
        auto requantized_expr = Requantize(quantized_expr, input_shape, input_scale, input_zero_point, output_scale, output_zero_point, input_dtype);
  ...
  else // for example, input_scales is Tuple([Constant(0.1574291)...
  {
